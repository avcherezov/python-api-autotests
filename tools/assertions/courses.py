...
def assert_get_cource_response(
        get_cource_response: GetCourseResponseSchema,
        create_cource_response: CreateCourseResponseSchema
):
    """
    Проверяет, что ответ на получение курсов соответствует ответу на его создание.

    :param get_cource_response: Ответ API при запросе данных курса.
    :param create_cource_response: Ответ API при создании курса.
    :raises AssertionError: Если данные задания не совпадают.
    """
    logger.info("Check get course response")

    assert_course(get_cource_response.course, create_cource_response.course)
...
def assert_cource_not_found_response(actual: InternalErrorResponseSchema):
    """
    ...
    """
    logger.info("Check cource not found response")
